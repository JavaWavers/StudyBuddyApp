...
public class ActiveUser {
  ...
  public static List<Exam> getExamForSubject(int subjectID) {
    ...
    String sql =
        "SELECT e.deadline, e.pages, e.revisionPerXPages, e.minutesPer20Slides, s.subjectName "
  ...
  public static List<ScheduledTask> getTasksForDay(int dayID) {
    String sql =
        "SELECT taskName, hoursAllocated, timeStarted, timeCompleted, taskStatus, taskDate, subjectName, taskType FROM Task WHERE dayID = ?";
    List<ScheduledTask> scheduledTasks = new ArrayList<>();
