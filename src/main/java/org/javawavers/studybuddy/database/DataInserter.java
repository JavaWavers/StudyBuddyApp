...
public class DataInserter {
  ...
  public static void insertSubject(
    ...
    String sql =
        "INSERT INTO Subject (subjectName, difficultyLevel, subjectType, userID) VALUES (?, ?, ?, ?);";
    try (Connection c = DataBaseManager.connect();
        PreparedStatement ps = c.prepareStatement(sql)) {
  ...
  public static void insertAssignment(
      ...
      int userID) {
    String sql =
        "INSERT INTO Assignment (title, deadline, estimateHours, difficulty, completedDate, userID) VALUES (?, ?, ?, ?, ?, ?);";
    try (Connection c = DataBaseManager.connect();
        PreparedStatement ps = c.prepareStatement(sql)) {
  ...
  public static void insertExam(
      /*String examName*/ LocalDate deadline,
      int pages,
      int revisionPerXPages,
      double minutesPer20Slides,
      int subjectID) {
    String sql =
        "INSERT INTO Exam (deadline, pages, revisionPerXPages, minutesPer20Slides, subjectID) VALUES (?, ?, ?, ?, ?);";
    try (Connection c = DataBaseManager.connect();
        PreparedStatement ps = c.prepareStatement(sql)) {
  ...
  public static void insertAvailability(
      ...
      int userID) {
    String sql =
        "INSERT INTO Availability (mondayAv, tuesdayAv, wednesdayAv, thursdayAv, fridayAv, saturdayAv, sundayAv, userID) VALUES (?, ?, ?, ?, ?, ?, ?, ?);";
    try (Connection c = DataBaseManager.connect();
        PreparedStatement ps = c.prepareStatement(sql)) {
